...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .stat-grid { display: flex; gap: 6px; margin-bottom: 10px; }
        .stat-box { flex: 1; text-align: center; padding: 5px 3px; border-radius: 3px; border: 1px solid #ddd; font-size: 8px; }
        .stat-label { font-size: 7.5px; color: #666; text-transform: uppercase; }
...
<div class="header">
    <h2>REKAP ABSENSI SISWA &mdash; {{ $mapel }}</h2>
...
<div class="info-grid">
    ...
    <div>
        ...
        <div class="info-row"><span class="info-label">Rata-rata Kelas</span><span>: {{ $rekapData['statistik']['rata_rata_kelas'] ?? 0 }}%</span></div>
        <div class="info-row"><span class="info-label">Tgl Cetak</span><span>: {{ \Carbon\Carbon::now()->format('d-M-Y H:i') }}</span></div>
</div>

@php
    $pred = $rekapData['statistik']['predikat'] ?? [];
@endphp
<div class="stat-grid">
    <div class="stat-box" style="background:#e8f5e9; border-color:#a5d6a7;">
        <div class="stat-label">Sangat Baik (≥90%)</div>
        <div class="stat-val" style="color:#2e7d32;">{{ $pred['sangat_baik'] ?? 0 }}</div>
    <div class="stat-box" style="background:#e3f2fd; border-color:#90caf9;">
        <div class="stat-label">Baik (75-89%)</div>
        <div class="stat-val" style="color:#1565c0;">{{ $pred['baik'] ?? 0 }}</div>
    <div class="stat-box" style="background:#fff9c4; border-color:#fff176;">
        <div class="stat-label">Cukup (60-74%)</div>
        <div class="stat-val" style="color:#f57f17;">{{ $pred['cukup'] ?? 0 }}</div>
    <div class="stat-box" style="background:#fce4ec; border-color:#ef9a9a;">
        <div class="stat-label">Kurang (<60%)</div>
        <div class="stat-val" style="color:#c62828;">{{ $pred['kurang'] ?? 0 }}</div>
</div>

@php
    $rekapSiswa = $rekapData['rekap_siswa'] ?? collect();
@endphp
...
<table>
    <thead>
        <tr class="thead-main">
            ...
            <th colspan="{{ count($bulanList) * 4 }}">Rekap Per Bulan ({{ $mapel }})</th>
    ...
    <tbody>
        @foreach ($rekapSiswa as $idx => $item)
        <tr class="{{ $idx % 2 === 1 ? 'row-even' : '' }}">
            ...
            <td class="total-h">{{ $item['hadir'] ?? 0 }}</td>
            <td class="total-s">{{ $item['sakit'] ?? 0 }}</td>
            <td class="total-i">{{ $item['izin'] ?? 0 }}</td>
            <td class="total-a">{{ $item['alpha'] ?? 0 }}</td>

            <td class="{{ ($item['persentase'] ?? 0) >= 90 ? 'pct-great' : (($item['persentase'] ?? 0) >= 75 ? 'pct-good' : (($item['persentase'] ?? 0) >= 60 ? 'pct-medium' : 'pct-bad')) }}">
                {{ $item['persentase'] ?? 0 }}%
            </td>
        </tr>
        @endforeach
        ...
        {{-- Baris JUMLAH --}}
        @php
            $jmlH = collect($rekapSiswa)->sum('hadir');
            $jmlS = collect($rekapSiswa)->sum('sakit');
            $jmlI = collect($rekapSiswa)->sum('izin');
            $jmlA = collect($rekapSiswa)->sum('alpha');
        ...
        <tr style="background:#e8eaf6; font-weight:bold;">
            ...
            @foreach ($rentangBulan as $bln)
                @php
                    $bH = collect($rekapSiswa)->sum(fn($d) => $d['per_bulan'][$bln]['h'] ?? 0);
                    $bS = collect($rekapSiswa)->sum(fn($d) => $d['per_bulan'][$bln]['s'] ?? 0);
                    $bI = collect($rekapSiswa)->sum(fn($d) => $d['per_bulan'][$bln]['i'] ?? 0);
                    $bA = collect($rekapSiswa)->sum(fn($d) => $d['per_bulan'][$bln]['a'] ?? 0);
                @endphp
        ...
        <tr style="background:#f3e5f5;">
            ...
            @foreach ($rentangBulan as $bln)
                @php
                    $bH  = collect($rekapSiswa)->sum(fn($d) => $d['per_bulan'][$bln]['h'] ?? 0);
                    $bAll = $bH + collect($rekapSiswa)->sum(fn($d) => ($d['per_bulan'][$bln]['s']??0)+($d['per_bulan'][$bln]['i']??0)+($d['per_bulan'][$bln]['a']??0));
