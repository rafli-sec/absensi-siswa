...
class RekapController extends Controller
{
    ...
    private function getRekapPerKelas($idGuru, $kelas, $rentangBulan, $tahunAjaran)
    {
        ...
        $rekapSiswa = $daftarSiswa->map(function ($siswa) use ($semuaAbsensi, $daftarMapel, $totalPertemuanPerMapel) {
            ...
            foreach ($daftarMapel as $mapel) {
                ...
                $perMapel[] = [
                    'mapel' => $mapel, 'hadir' => $hadir, 'sakit' => $sakit,
                    'izin'  => $izin,  'alpha' => $alpha,
                    'total_pertemuan' => $totalPertemuan, 'persentase' => $persentase,
                ];
            }
            ...
            return [
                'id_siswa' => $siswa->id_siswa, 'nis' => $siswa->nis,
    ...
    public function getRekapPerMapel($idGuru, $kelas, $mapel, $rentangBulan, $tahunAjaran)
    {
        ...
        return [
            'type'         => 'mapel', 'kelas' => $kelas, 'mapel' => $mapel,
            'tahun_ajaran' => $tahunAjaran, 'total_pertemuan' => $totalPertemuan,
    ...
    private function getRekapPerSiswa($idSiswa, $idGuru, $rentangBulan, $tahunAjaran)
    {
        ...
        if ($semuaAbsensi->isEmpty()) {
            return [
                'type' => 'siswa', 'siswa' => $siswa, 'tahun_ajaran' => $tahunAjaran,
        ...
        foreach ($daftarMapel as $mapel) {
            ...
            $rekapMapel[] = [
                'mapel' => $mapel, 'hadir' => $hadir, 'sakit' => $sakit,
                'izin'  => $izin,  'alpha' => $alpha,
                'persentase' => $persentase, 'total_pertemuan' => $totalPertemuan,
        ...
        return [
            ...
            'tahun_ajaran' => $tahunAjaran,
            'total_hadir'  => $totalHadirSemua, 'total_sakit' => $totalSakitSemua,
            'total_izin'   => $totalIzinSemua,  'total_alpha' => $totalAlphaSemua,
    ...
    // =========================================================
    //  EXPORT PDF - REKAP PER KELAS PER BULAN (semua mapel)
    // =========================================================
    public function exportKelasBulanPDF(Request $request)
    {
        ...
        // Ambil semua absensi bulan tersebut, semua mapel
        $daftarSiswa = Siswa::where('kelas', $kelas)->where('status', 'aktif')
            ->orderBy('nama_siswa')->get();

        $semuaAbsensi = Absensi::where('id_guru', $idGuru)
            ->whereIn(DB::raw('MONTH(tanggal)'), [$bulan])
            ->whereHas('siswa', fn($q) => $q->where('kelas', $kelas))
            ->get();

        // Daftar mapel di bulan ini
        $daftarMapel = $semuaAbsensi->pluck('mapel')->unique()->sort()->values();

        // Tanggal pertemuan di bulan itu (semua mapel digabung)
        $tanggalPertemuan = $semuaAbsensi->pluck('tanggal')->unique()->sort()->values();
        $totalPertemuan   = $tanggalPertemuan->count();

        // Jumlah hari dalam bulan
        $tahunPdf  = (int) explode('/', $tahunAjaran)[0];
        // Semester genap → tahun kedua
        $tahunReal = $bulan <= 6 ? $tahunPdf + 1 : $tahunPdf;
        $jumlahHari   = cal_days_in_month(CAL_GREGORIAN, $bulan, $tahunReal);
        ...
        // Rekap per siswa: per mapel + status per tanggal
        $detailSiswa = $daftarSiswa->map(function ($siswa) use ($semuaAbsensi, $tanggalPertemuan, $daftarMapel) {
            $absSiswa = $semuaAbsensi->where('id_siswa', $siswa->id_siswa);

            // Status per tanggal (semua mapel digabung — ambil status pertama yang ditemukan per tanggal)
            $statusByDate = [];
            $h = $s = $i = $a = 0;
            foreach ($tanggalPertemuan as $tgl) {
                $tglFormatted = Carbon::parse($tgl)->format('Y-m-d');
                $abs    = $absSiswa->firstWhere('tanggal', $tgl);
                $status = $abs ? $abs->status_kehadiran : '-';
                $statusByDate[$tglFormatted] = $status;
                if ($status === 'hadir')      $h++;
            ...
            // Per mapel untuk bulan ini
            $perMapel = [];
            foreach ($daftarMapel as $mapel) {
                $absMapel    = $absSiswa->where('mapel', $mapel);
                $mh = $absMapel->where('status_kehadiran', 'hadir')->count();
                $ms = $absMapel->where('status_kehadiran', 'sakit')->count();
                $mi = $absMapel->where('status_kehadiran', 'izin')->count();
                $ma = $absMapel->where('status_kehadiran', 'alpha')->count();
                $perMapel[$mapel] = ['h' => $mh, 's' => $ms, 'i' => $mi, 'a' => $ma];
            }
    ...
    // =========================================================
    //  EXPORT PDF - REKAP PER KELAS PER SEMESTER (semua mapel)
    // =========================================================
    public function exportKelasSemesterPDF(Request $request)
    {
        ...
        // Rekap per siswa per bulan (semua mapel digabung)
        $detailSiswa = $daftarSiswa->map(function ($siswa) use ($semuaAbsensi, $rentangBulan) {
            $absSiswa = $semuaAbsensi->where('id_siswa', $siswa->id_siswa);
            $perBulan = [];
            ...
            return [
                ...
                'jenis_kelamin' => $siswa->jenis_kelamin,
                'per_bulan'     => $perBulan,
                'total_h'       => $totalH, 'total_s' => $totalS,
                'total_i'       => $totalI, 'total_a' => $totalA,
                'total_pertemuan' => $totalPertemuan, 'pct_total' => $pctTotal,
            ];
        });
    ...
    // =========================================================
    //  EXPORT PDF - REKAP PER MAPEL PER SEMESTER
    // =========================================================
    public function exportMapelPDF(Request $request)
    {
        ...
        // Tambahkan data per bulan ke setiap siswa
        $semuaAbsensi = Absensi::where('id_guru', $idGuru)->where('mapel', $mapel)
            ->whereIn(DB::raw('MONTH(tanggal)'), $rentangBulan)
            ->whereHas('siswa', fn($q) => $q->where('kelas', $kelas))->get();

        $rekapSiswaEnriched = collect($rekapData['rekap_siswa'])->map(function ($siswa) use ($semuaAbsensi, $rentangBulan) {
            $absSiswa = $semuaAbsensi->where('id_siswa', $siswa['id_siswa']);
            $perBulan = [];
            foreach ($rentangBulan as $bln) {
                $absBulan   = $absSiswa->filter(fn($ab) => Carbon::parse($ab->tanggal)->month === $bln);
        ...
        $pdf = Pdf::loadView('pdf.rekap_mapel', [
            ...
            'bulanList'    => $bulanList,
            'rentangBulan' => $rentangBulan,
            'rekapData'    => array_merge($rekapData, ['rekap_siswa' => $rekapSiswaEnriched]),
        ])->setPaper('a4', 'landscape');
    ...
    // =========================================================
    //  EXPORT PDF - REKAP PER MAPEL PER BULAN (BARU)
    // =========================================================
    public function exportMapelBulanPDF(Request $request)
    {
        $kelas       = $request->kelas;
        $mapel       = $request->mapel;
        $bulan       = (int) $request->bulan;
        ...
        $detailSiswa = $daftarSiswa->map(function ($siswa) use ($semuaAbsensi, $tanggalPertemuan) {
            $absSiswa     = $semuaAbsensi->where('id_siswa', $siswa->id_siswa);
            $statusByDate = [];
            $h = $s = $i = $a = 0;
        ...
        $pdf = Pdf::loadView('pdf.rekap_mapel_bulan', [
            'kelas'           => $kelas,
            'mapel'           => $mapel,
            'bulan'           => $bulan,
            'nama_bulan'      => ucfirst($namaBulan),
        ...
        return $pdf->download("Rekap_Mapel_{$mapel}_{$kelas}_{$namaBulan}_{$tahunAjaranSafe}.pdf");
    }

    // =========================================================
    //  EXPORT PDF - REKAP PER SISWA PER SEMESTER
    // =========================================================
    public function exportSiswaSemesterPDF(Request $request)
    {
        ...
        $rekapSemuaSiswa = $daftarSiswa->map(function ($siswa) use ($semuaAbsensi, $daftarMapel, $totalPertemuanPerMapel, $rentangBulan) {
            ...
            return [
                'nis' => $siswa->nis, 'nama_siswa' => $siswa->nama_siswa,
